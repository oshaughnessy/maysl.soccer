...
make serve        # jekyll serve with livereload on port 4000 (uses local.yml)
make update       # bundle update --all
make lint         # yamllint _config.yml
make local-config # generate local.yml for local dev (replaces remote_theme with local gem)
```

Run `make local-config` once before `make serve` � it generates `local.yml` which swaps the remote theme reference for the locally-installed gem.
...
- **Quoted `{% include %}` params are never Liquid-rendered**, and nested quotes
  inside them break Jekyll's param parser outright. Build the string with
  `capture` first and pass the variable unquoted.
- **Liquid tokenizes tags inside `{% comment %}` blocks.** An unmatched
  `{% capture %}` in a comment is a parse error.
...
`_config.yml` uses `remote_theme` (for GitHub Pages). `local.yml` overrides that with `theme: minimal-mistakes-jekyll` for local builds. `local.yml` is generated by `make local-config` and is git-ignored � don't commit it.
